  ...
  return (
    <div className="space-y-8 text-center">
      ...
      {isGift && startkaartUrl && (
        <div className={`rounded-2xl p-6 text-left space-y-4 ${isBaby ? `bg-white border ${babyT.primaryBorderMedium}` : "bg-[#1a1a1a]"}`}>
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Printer className={`h-5 w-5 ${isBaby ? babyT.primaryText : "text-[#d4af37]"}`} />
              <h3 className={`font-serif font-bold text-lg ${isBaby ? babyT.primaryText : "text-[#d4af37]"}`}>Iets voor {who}</h3>
            </div>
            <p className={`text-sm mb-4 ${isBaby ? "text-[#666]" : "text-[#bbb]"}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* Cadeaubon (A4 PDF) */}
            <div className={`rounded-xl p-4 border ${isBaby ? `${babyT.primaryBg} ${babyT.primaryBorder}` : "bg-white/5 border-white/10"}`}>
              <div className="flex items-center gap-2 mb-1">
                <FileText className={`h-4 w-4 ${isBaby ? babyT.primaryText : "text-[#d4af37]"}`} />
                <p className={`text-sm font-semibold ${isBaby ? "text-[#1a1a1a]" : "text-white"}`}>Cadeaubon (A4)</p>
              </div>
              <p className={`text-xs mb-3 leading-relaxed ${isBaby ? "text-[#666]" : "text-[#999]"}`}>
            ...
            <div className={`rounded-xl p-4 border ${isBaby ? `${babyT.primaryBg} ${babyT.primaryBorder}` : "bg-white/5 border-white/10"}`}>
              <div className="flex items-center gap-2 mb-1">
                <Printer className={`h-4 w-4 ${isBaby ? babyT.primaryText : "text-[#aaa]"}`} />
                <p className={`text-sm font-semibold ${isBaby ? "text-[#1a1a1a]" : "text-white"}`}>Digitale startkaart</p>
              </div>
              <p className={`text-xs mb-3 leading-relaxed ${isBaby ? "text-[#666]" : "text-[#999]"}`}>
              ...
              <button
                ...
                className={`w-full font-medium px-4 py-2.5 rounded-lg transition-colors text-sm border ${isBaby ? babyT.outlineBtn : "border-white/20 text-white hover:bg-white/10"}`}
              >
                Bekijk startkaart →
              </button>
            </div>
